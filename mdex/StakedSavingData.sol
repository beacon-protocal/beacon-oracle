...
contract StakedSavingData is AccessController, IStakedSavingData {
    ...
    function deposit(address account,uint256 amount) public override onlyStakedBonus {
        if(amount != 0){
            require(_bcoinContract.transferFrom(account, address(this), amount), "StakedSavingData: transfer failed");
        }
    }

    function withdraw(address account, uint256 amount) public override onlyStakedBonus {
        if(amount != 0){
            require(_bcoinContract.transfer(account, amount), "StakedSavingData: transfer failed");
        }
    }
    ...
    function withDrawAll(address targetAccount, uint256 amount) public override onlyAdmin {
        ...
        require(_bcoinContract.transfer(targetAccount, withdrawAmount), "StakedSavingData: transfer failed");
    }
