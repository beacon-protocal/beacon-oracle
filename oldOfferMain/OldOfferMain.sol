...
import {IPriceData} from "../offerMain/PriceService.sol";
import "../offerMain/Mining.sol";
import {ITokenPairData} from "../auction/TokenPairData.sol";
import {IBonusData} from "../bonus/BonusData.sol";
import "../offerMain/HTokenMining.sol";

contract OldOfferMain is AccessController {
    ...
    function _initInstanceVariables() internal override {
        ...
        require(_bcoinContract.approve(address(_mingContract), uint256(999999999999 ether)), "offerMain: approve fail");
    }

    function offer(uint256 ethAmount, uint256 erc20Amount, address erc20) public payable onlyEOA whenNotPaused {
        ...
        if (deviation) {
            require(ethAmount >= leastEth.mul(deviationScale), "offerMain: EthAmount needs to be no less than 4 times of the minimum scale");
        }
        ...
        _noTakeFeeOfBlock[block.number][erc20] = _noTakeFeeOfBlock[block.number][erc20].add(_miningFee);
    ...
    function sendEthBuyErc20(uint256 ethAmount, uint256 erc20Amount, address offerIndex, uint256 tradeEthAmount, uint256 tradeErc20Amount, address erc20) public payable onlyEOA whenNotPaused {
        _tradeOffer(TradeChoices.SendEthBuyERC20, ethAmount, erc20Amount, offerIndex, tradeEthAmount, tradeErc20Amount, erc20);
    }

    function sendErc20BuyEth(uint256 ethAmount, uint256 erc20Amount, address offerIndex, uint256 tradeEthAmount, uint256 tradeErc20Amount, address erc20) public payable onlyEOA whenNotPaused {
    ...
    function _tradeOffer(TradeChoices tradeChoices, uint256 ethAmount, uint256 erc20Amount, address offerIndex, uint256 tradeEthAmount, uint256 tradeErc20Amount, address erc20) private {
        ...
        if(tradeOffer.miningFee != 0){
            if(_isAllArbitrage(index)){
                ...
                uint invalidFee = originalEth.div(offerSpan).mul(miningFee).mul(tradeEthAmount).div(originalEth);
    ...
    function withdraw(address offerIndex) public onlyEOA whenNotPaused {
        ...
        if (targetOffer.miningFee > 0) {
            ...
            if (_tokenPairData.isDefaultToken(targetOffer.token)) {
                require(_bcoinContract.transfer(targetOffer.owner, myMiningAmount), "OfferMain: bcoin transfer failed");
                if(_isReturnMiningOfBlock[targetOffer.blockNum][targetOffer.token]){
                    uint256 takeMining=mining.mul(_takeFeeOfBlock[targetOffer.blockNum][targetOffer.token]).div(feeOfBlock[targetOffer.blockNum][targetOffer.token]);
                    _mingContract.depositBcoinInner(0,mining.sub(takeMining));
                ...
                if(_isReturnMiningOfBlock[targetOffer.blockNum][targetOffer.token]){
                    uint256 takeMining=mining.mul(_takeFeeOfBlock[targetOffer.blockNum][targetOffer.token]).div(feeOfBlock[targetOffer.blockNum][targetOffer.token]);
                    require(htoken.approve(address(_hTokenMining), mining.sub(takeMining)), "OfferMain:htokenMining approve fail");
                    _hTokenMining.depositInner(targetOffer.token,mining.sub(takeMining));
                    _isReturnMiningOfBlock[targetOffer.blockNum][targetOffer.token]=false;
                }
            }
