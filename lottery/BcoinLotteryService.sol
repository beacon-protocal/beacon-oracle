...
import "./BcoinLottery.sol";
import "./BuyRecord.sol";

interface IBcoinLotteryService {
    // buy lottery
...
contract BcoinLotteryService is IBcoinLotteryService,AccessController {
    using SafeMath for uint256;

    BuyRecord private _buyRecordContract;
    BcoinLottery private _bcoinLotteryContract;
    ...
    function _initInstanceVariables() internal override {
        ...
        _bcoinLotteryContract = BcoinLottery(getContractAddress("bcoinLottery"));
        _winningRecordContract = WinningRecord(getContractAddress("bcoinWinningRecord"));
        super.addGames(address(this));
    }
    // buy lottery
    function buyLottery(uint256 first_number,uint256 second_number,uint256 third_number,uint256 forth_number,uint256 noteNumber,address account) public override {
        _bcoinLotteryContract.buyLottery(first_number,second_number,third_number,forth_number,noteNumber,account);
    }
