...
contract Bonus is AccessController, IBonus {
    ...
    uint256 public expectSpanForBcoin = 1000000 ether;
    uint256 public expectSpanForHToken = 1000000 ether;
    //主网husd 精度为8位
    uint256 miniUSDTBonus = 5000000000;
    ...
    function lockup(uint256 amount, address token) external override whenNotPaused returns (bool) {
    ...
    function unlock(address token, uint256 amount) public override whenNotPaused returns(bool) {
        require(_isValidToken(token), "bonus: invalid token");
    ...
    function receiveBonus(address token) public override whenNotPaused returns(bool){
        require(_isValidToken(token), "bonus: invalid token");
    ...
    function setNextTime(uint256 nextTime) public onlyAdmin whenNotPaused {
    ...
    function setDistributionTime(uint256 distributionTime) public onlyAdmin whenNotPaused {
    ...
    function _getTotalCirculation(address token) public view returns (uint256){
        if (token == address(address(_bcoinContract))) {
            uint256 totalSupply = _bcoinContract.totalSupply();
            uint256 destroyed = _bcoinContract.balanceOf(_destructionAddress);
            uint256 miningPool = _bcoinContract.balanceOf(getContractAddress("mining"));
...
//        return nowTime >= nextTime && nowTime <= nextTime + bonusTimeLimit;
    ...
    function _getExpectedMiniBonus(uint256 _miniBonus,address token) public view returns (uint256) {
        ...
        uint256 steps;
        if (token == address(_bcoinContract)) {
            steps = circulation.div(expectSpanForBcoin);
        } else {
    ...
    function _isValidToken(address token) private view returns (bool){
        return _tokenPairDataContract.isHToken(token) || token == address(_bcoinContract);
    }
